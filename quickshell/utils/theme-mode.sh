...
CURRENT_WALLPAPER="$HOME/.local/state/theme/current_wallpaper"
if [[ -f "$CURRENT_WALLPAPER" ]]; then
  WALLPAPER_DARK="$CURRENT_WALLPAPER"
  WALLPAPER_LIGHT="$CURRENT_WALLPAPER"
fi
# PATHS
...
if [[ "$QUIET" != "1" ]]; then
  ...
  # Set the static fallback first; pywal replaces it below when wallpaper
  # colours are enabled.
  ...
  if [[ "$NO_WALLPAPER" != "1" && -f "$wallpaper" ]]; then
    ...
    "$HOME/.config/hypr/scripts/pywal.sh" "$wallpaper" "$mode" >/dev/null 2>&1 || true
  fi
