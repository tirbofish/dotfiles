...
Lib.Card {
  ...
  Popup {
    ...
    height: Math.min(audioPicker.implicitHeight + 24, 420)
    padding: 12
    focus: true
    closePolicy: Popup.CloseOnEscape | Popup.CloseOnPressOutside
    background: Rectangle {
      radius: 14; color: root.theme.bgCard
      border.width: 1; border.color: root.theme.outline
    }
    contentItem: Flickable {
      contentWidth: width; contentHeight: audioPicker.implicitHeight
      clip: true; boundsBehavior: Flickable.StopAtBounds
      Lib.AudioDevicePicker { id: audioPicker; width: parent.width; theme: root.theme }
    }
    onOpened: Lib.AudioService.refresh()
  }
  ...
  function openWifiMenu() {
    var screenName = Window.window && Window.window.screen ? Window.window.screen.name : ""
    Lib.Overlays.openWifi(screenName)
    root.closeRequested()
  }
  // --------------------------------------------------------------------------------------------------------------
  // BLUETOOTH 
  ...
  Lib.CommandPoll {
    ...
    command: Lib.Shell.sh("pw-dump | jq -r '[.[] | select(.type == \"PipeWire:Interface:Device\" and ((.info.props[\"device.name\"] // \"\") | startswith(\"bluez_card\"))) | .info.props[\"device.description\"]] | first // empty'")
    parse: function(o) {
      var d = String(o).trim();
  ...
  Lib.CommandPoll {
    ...
    command: Lib.Shell.sh("(wpctl get-volume " + (Lib.AudioService.volumeNodeId >= 0 ? Lib.AudioService.volumeNodeId : "@DEFAULT_AUDIO_SINK@") + " 2>/dev/null | grep '^Volume:' || wpctl get-volume @DEFAULT_AUDIO_SINK@ 2>/dev/null) | awk '{print int($2 * 100 + .5)}'")
  ...
  function _applyPerf(mode) {
    ...
    perfProc.command = ["pkexec", "auto-cpufreq", "--force=" + mode]
  ...
  function getPerfColor() {
    if (root.autoMode) return root.theme ? root.theme.accent : Theme.accent
    ...
    RowLayout {
        ...
        Item {
            ...
            Lib.ExpressiveSlider {
                ...
                from: 1;
                to: 100; value: 50
                accentColor: root.theme ? root.theme.accentSlider : Theme.accent
        ...
        Item {
            ...
            Lib.ExpressiveSlider {
                ...
                to: 100; value: 0
                accentColor: root.theme ? root.theme.accentSlider : Theme.accent
                onUserChanged: Lib.Shell.det("if wpctl get-volume " + (Lib.AudioService.volumeNodeId >= 0 ? Lib.AudioService.volumeNodeId : "@DEFAULT_AUDIO_SINK@") + " 2>/dev/null | grep -q '^Volume:'; then wpctl set-volume " + (Lib.AudioService.volumeNodeId >= 0 ? Lib.AudioService.volumeNodeId : "@DEFAULT_AUDIO_SINK@") + " " + Math.round(value) + "%; else wpctl set-volume @DEFAULT_AUDIO_SINK@ " + Math.round(value) + "%; fi")
                onRightClicked: audioPopup.open()
            } 
        }
    }
  }
