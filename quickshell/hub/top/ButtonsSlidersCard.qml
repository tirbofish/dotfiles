...
Lib.TopCard {
  ...
  Lib.CommandPoll {
    ...
    command: sh("pw-dump | jq -r '[.[] | select(.type == \"PipeWire:Interface:Device\" and ((.info.props[\"device.name\"] // \"\") | startswith(\"bluez_card\"))) | .info.props[\"device.description\"]] | first // empty'")
    parse: function(o) {
      var d = String(o).trim();
  ...
  Lib.CommandPoll {
    ...
    running: root.active && root.visible
    interval: 1200
    command: sh("(wpctl get-volume " + (Lib.AudioService.volumeNodeId >= 0 ? Lib.AudioService.volumeNodeId : "@DEFAULT_AUDIO_SINK@") + " 2>/dev/null | grep '^Volume:' || wpctl get-volume @DEFAULT_AUDIO_SINK@ 2>/dev/null) | awk '{print int($2 * 100 + .5)}'")
    parse: function(o) {
...
// --- CPU GOVERNOR (AUTO-CPUFREQ) ---
  ...
  function getPerfColor() {
    if (root.autoMode) {
      return root.theme ? root.theme.accent : "#7aa1a6"
    }
  ...
  ColumnLayout {
    ...
    Layout.fillWidth: true
      ...
      Lib.ExpressiveSlider {
        ...
        value: 50
        Layout.fillWidth: true
        accentColor: root.theme ? root.theme.accentSlider : "#7aa1a6"
