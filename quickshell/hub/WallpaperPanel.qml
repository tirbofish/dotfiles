...
Item {
    ...
    readonly property color rAccent:  theme ? theme.accent         : "#7aa1a6"
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: (data) => {
                var colors = data.toString().trim().split(/\s+/)
                if (colors.length < 5) return
                Lib.Configuration.customBg = colors[0]
                Lib.Configuration.customForeground = colors[1]
                Lib.Configuration.customAccent = colors[2]
                Lib.Configuration.customSecondary = colors[3]
                Lib.Configuration.customDanger = colors[4]
                Lib.Configuration.taskbarAccent = colors[2]
                Lib.Configuration.useCustomColors = true
                Lib.Configuration.save()
            }
        }
        onExited: (code) => root.triggerToast(code === 0 ? "wallpaper colours applied" : "pywal failed")
    }
                    ...
                    Repeater {
                        ...
                        delegate: WpThumbnail {
                            ...
                            onApplyRequested: {
                                ...
                                Quickshell.execDetached(["ln", "-sfn", model.full_path,
                                    Quickshell.env("HOME") + "/.local/state/theme/current_wallpaper"])
                                backend.write("apply:" + model.full_path + "\n")
                                backend.flush()
                                pywalProc.running = false
                                pywalProc.command = [Quickshell.env("HOME") + "/.config/hypr/scripts/pywal.sh",
                                                     model.full_path, root.rDark ? "dark" : "light"]
                                pywalProc.running = true
                                root.triggerToast("applying wallpaper…")
