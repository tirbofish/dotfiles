...
PanelWindow {
    ...
    // audiocontrol.sh resolves the sink volume should actually apply to
    // (the enhanced Surface speaker profile is a virtual passthrough sink
    // that always reports 100%, so plain pamixer/pactl calls against the
    // default sink never move the real, audible volume).
    readonly property string audioControlScript: Quickshell.env("HOME") + "/.config/hypr/scripts/audiocontrol.sh"

    Process {
        id: muteProc
        command: ["bash", "-lc", root.audioControlScript + " m"]
        running: false
    }
    ...
    Timer {
        ...
        onTriggered: {
            ...
            setVolProc.command = ["bash", "-lc", root.audioControlScript + " s " + pendingVol]
